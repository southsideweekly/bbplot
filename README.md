...
This repo contains the functions of the `sswplot` package, which once installed locally, provides helpful functions for creating and exporting  graphics made in ggplot in the style used by the South Side Weekly data team. `sswplot` is a fork of the [`bbplot` package developed by the BBC](https://github.com/bbc/bbplot).

![Example of graphics created using the sswplot package](chart_examples/bbplot_example_plots.png)
...
1. `ssw_style()`: has no arguments and is added to the ggplot chain after you have created a plot. What it does is generally makes text size, font and colour, axis lines, axis text and many other standard chart components into SSW style.
